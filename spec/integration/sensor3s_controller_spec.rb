...
RSpec.describe "Sensor1s API", :type => :request do
  path "/api/v1/things/{id}/sensor3s" do
    get 'Retrieves all Sensor3s of a Thing' do
      tags 'Sensor3s'
      ...
      response '200', 'all accumulators ' do
        ...
        let(:sensor3)     { create(:sensor3) }
        let(:thing)       { sensor3.uplink.thing }
        let(:id)          { sensor3.uplink.thing.id }
